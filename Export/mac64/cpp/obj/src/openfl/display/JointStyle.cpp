...
int JointStyle_obj::__FindIndex(::String inName)
{
	...
	if (inName==HX_HCSTRING("ROUND","\x2e","\x30","\x54","\x6b")) return 1;
...
hx::Static(__mClass) = hx::RegisterClass(HX_HCSTRING("openfl.display.JointStyle","\x71","\x4c","\x9d","\x48"), hx::TCanCast< JointStyle_obj >,sStaticFields,sMemberFields,
...
hx::Static(MITER) = hx::CreateEnum< JointStyle_obj >(HX_HCSTRING("MITER","\xe5","\x37","\x5b","\x86"),0);
hx::Static(ROUND) = hx::CreateEnum< JointStyle_obj >(HX_HCSTRING("ROUND","\x2e","\x30","\x54","\x6b"),1);
}
